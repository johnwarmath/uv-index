...
- **Progress tracking**: tasks per site with status and % complete, grouped by the same Flow → Stage taxonomy as QAQC (e.g. Foundations → Piles, Rows → Finish Racking), so progress and quality checks line up against the same structure. Each Flow section shows a rolled-up progress bar; each Stage groups its tasks underneath. When creating a new site, you pick exactly which of the 239 checklist items are relevant to that project — only the ones you toggle on become initial Progress tasks (grouped by Flow/Stage with select-all shortcuts, since not every project needs every stage).
