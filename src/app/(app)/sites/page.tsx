...
export default async function SitesPage({
  ...
  const archivedCount = allSites.filter((s) => s.archived).length;
  const siteList = showArchived ? allSites.filter((s) => s.archived) : allSites.filter((s) => !s.archived);
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="font-display text-3xl font-semibold">{showArchived ? 'Archived sites' : 'Sites'}</h1>
        </div>
      ...
      {archivedCount > 0 && (
        <div className="mb-6">
          <Link
            href={showArchived ? '/sites' : '/sites?archived=1'}
            className="text-sm text-[var(--color-amber)] hover:underline"
          >
            {showArchived ? '← Back to active sites' : `View archived sites (${archivedCount})`}
          </Link>
        </div>
      )}
      ...
      {siteList.length === 0 ? (
        <div className="rounded-lg border border-dashed border-[var(--color-border)] p-12 text-center">
          <p className="text-[var(--color-paper-dim)] mb-1">
